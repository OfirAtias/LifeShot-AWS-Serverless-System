...
# + ✅ NEW: Split Detector into 3 Lambdas (Detector / RenderAndS3 / EventsAndSNS)
#
...
# - Uses EXISTING Events API Lambda (default: LifeShot_Api_Handler)  <-- API Gateway /events stays here
...
# -------------------------
# ✅ NEW CONFIG: Split Detector Lambdas (3 functions)
# -------------------------
# Existing "API events" lambda (used by API Gateway /events routes) stays as-is:
EVENTS_API_LAMBDA_NAME="${EVENTS_API_LAMBDA_NAME:-LifeShot_Api_Handler}"

# Split lambdas:
DETECTOR_LAMBDA_NAME="${DETECTOR_LAMBDA_NAME:-LifeShot_detector_logic}"
RENDER_LAMBDA_NAME="${RENDER_LAMBDA_NAME:-LifeShot_RenderAndS3}"
EVENTS_SNS_LAMBDA_NAME="${EVENTS_SNS_LAMBDA_NAME:-LifeShot_EventsAndSNS}"

# Handlers (because you renamed files - not lambda_function.py)
DETECTOR_HANDLER="${DETECTOR_HANDLER:-detector_logic.lambda_handler}"
RENDER_HANDLER="${RENDER_HANDLER:-render_and_s3.lambda_handler}"
EVENTS_SNS_HANDLER="${EVENTS_SNS_HANDLER:-events_and_sns.lambda_handler}"

# Memory requirements
DETECTOR_MEMORY="${DETECTOR_MEMORY:-1024}"
RENDER_MEMORY="${RENDER_MEMORY:-1024}"
EVENTS_SNS_MEMORY="${EVENTS_SNS_MEMORY:-256}"

# Timeouts
DETECTOR_TIMEOUT="${DETECTOR_TIMEOUT:-60}"
RENDER_TIMEOUT="${RENDER_TIMEOUT:-45}"
EVENTS_SNS_TIMEOUT="${EVENTS_SNS_TIMEOUT:-15}"

API_NAME="${API_NAME:-${STACK_PREFIX}HttpApi}"

# (kept for backward-compat; not used when using LabRole)
...
log "Using LAB ROLE: $LAB_ROLE_ARN"

# Helper: merge env vars safely (no overwrite except keys we set)
merge_lambda_env() {
  local function_name="$1"
  local py_updates="$2"   # python snippet that mutates dict 'cur'
...
# -------------------------
# 1) Use LabRole for ALL Lambdas (no IAM creation in lab)
# -------------------------
ROLE_ARN="$LAB_ROLE_ARN"
log "Using role for Lambda: $ROLE_ARN"
...
# -------------------------
# 6) Generate LOGIN.MJS (with complete-password)
# -------------------------
WORKDIR="$(mktemp -d)"
...
# -------------------------
# ✅ 7.5) Ensure DETECTOR + RENDER + EVENTS(SNS) Lambdas exist + set handler/memory/env
# -------------------------
log "Ensuring split Lambdas exist + updating configs (Detector/Render/Events+SNS)"
...
RENDER_ARN="$(aws lambda get-function --region "$REGION" --function-name "$RENDER_LAMBDA_NAME" --query Configuration.FunctionArn --output text 2>/dev/null || true)"
[[ -z "${RENDER_ARN}" || "${RENDER_ARN}" == "None" ]] && die "Render Lambda not found: $RENDER_LAMBDA_NAME (create it first)"

EVENTS_SNS_ARN="$(aws lambda get-function --region "$REGION" --function-name "$EVENTS_SNS_LAMBDA_NAME" --query Configuration.FunctionArn --output text 2>/dev/null || true)"
[[ -z "${EVENTS_SNS_ARN}" || "${EVENTS_SNS_ARN}" == "None" ]] && die "Events+SNS Lambda not found: $EVENTS_SNS_LAMBDA_NAME (create it first)"

log "Updating Detector config: handler=$DETECTOR_HANDLER memory=${DETECTOR_MEMORY}MB timeout=${DETECTOR_TIMEOUT}s"
aws lambda update-function-configuration \
  --region "$REGION" \
  --function-name "$DETECTOR_LAMBDA_NAME" \
  --handler "$DETECTOR_HANDLER" \
  --memory-size "$DETECTOR_MEMORY" \
  --timeout "$DETECTOR_TIMEOUT" >/dev/null

log "Updating Render config: handler=$RENDER_HANDLER memory=${RENDER_MEMORY}MB timeout=${RENDER_TIMEOUT}s"
aws lambda update-function-configuration \
  --region "$REGION" \
  --function-name "$RENDER_LAMBDA_NAME" \
  --handler "$RENDER_HANDLER" \
  --memory-size "$RENDER_MEMORY" \
  --timeout "$RENDER_TIMEOUT" >/dev/null

log "Updating Events+SNS config: handler=$EVENTS_SNS_HANDLER memory=${EVENTS_SNS_MEMORY}MB timeout=${EVENTS_SNS_TIMEOUT}s"
aws lambda update-function-configuration \
  --region "$REGION" \
  --function-name "$EVENTS_SNS_LAMBDA_NAME" \
  --handler "$EVENTS_SNS_HANDLER" \
  --memory-size "$EVENTS_SNS_MEMORY" \
  --timeout "$EVENTS_SNS_TIMEOUT" >/dev/null

log "Updating Detector env vars (merge-safe): set RENDER_LAMBDA_NAME/EVENTS_LAMBDA_NAME and remove SNS_TOPIC_ARN..."
merge_lambda_env "$DETECTOR_LAMBDA_NAME" "
cur['RENDER_LAMBDA_NAME'] = '$RENDER_LAMBDA_NAME'
cur['EVENTS_LAMBDA_NAME'] = '$EVENTS_SNS_LAMBDA_NAME'
cur.pop('SNS_TOPIC_ARN', None)
"

# -------------------------
# ✅ 7.6) SNS: Create Topic + Access Policy + Subscribe Admin + Set SNS_TOPIC_ARN on Events+SNS Lambda
# -------------------------
...
log "Ensuring SNS email subscription exists (or pending) for: $ADMIN_EMAIL"
...
log "Setting Events+SNS Lambda env var SNS_TOPIC_ARN (merge-safe)..."
merge_lambda_env "$EVENTS_SNS_LAMBDA_NAME" "
cur['SNS_TOPIC_ARN'] = '$SNS_TOPIC_ARN'
"

log "Events+SNS env updated: SNS_TOPIC_ARN=$SNS_TOPIC_ARN"

# -------------------------
# ✅ 7.7) Ensure DETECTOR Function URL (Auth NONE + CORS)
# -------------------------
...
# ✅ Events API Lambda ARN (must already exist) - THIS is what API Gateway /events uses
log "Resolving Events API Lambda ARN: $EVENTS_API_LAMBDA_NAME"
EVENTS_API_ARN="$(aws lambda get-function --region "$REGION" --function-name "$EVENTS_API_LAMBDA_NAME" --query Configuration.FunctionArn --output text 2>/dev/null || true)"
[[ -z "${EVENTS_API_ARN}" || "${EVENTS_API_ARN}" == "None" ]] && die "Events API Lambda not found: $EVENTS_API_LAMBDA_NAME (create it first)"

log "Creating/Ensuring integration to Events API Lambda: $EVENTS_API_LAMBDA_NAME"
EVENTS_INTEGRATION_ID="$(aws apigatewayv2 get-integrations \
  --region "$REGION" \
  --api-id "$API_ID" \
  --query "Items[?IntegrationUri=='$EVENTS_API_ARN'].IntegrationId | [0]" \
  --output text 2>/dev/null || true)"
...
if [[ -z "${EVENTS_INTEGRATION_ID}" || "${EVENTS_INTEGRATION_ID}" == "None" ]]; then
  EVENTS_INTEGRATION_ID="$(aws apigatewayv2 create-integration \
    ...
    --api-id "$API_ID" \
    --integration-type AWS_PROXY \
    --integration-uri "$EVENTS_API_ARN" \
    --payload-format-version "2.0" \
    --query IntegrationId --output text)"
...
aws lambda add-permission \
  --region "$REGION" \
  --function-name "$EVENTS_API_LAMBDA_NAME" \
  --statement-id "${STACK_PREFIX}ApiGwInvokeEventsApi" \
  --action lambda:InvokeFunction \
  --principal apigateway.amazonaws.com \
  --source-arn "$SOURCE_ARN" >/dev/null 2>&1 || true

# -------------------------
# 10) Create JWT Authorizer (for /events only)
# -------------------------
...
# -------------------------
# 11) Create Routes + Attach Integrations
# -------------------------
log "Creating routes..."
...
# Events routes -> Events API Lambda integration + JWT (STAYS ON LifeShot_Api_Handler)
...
echo "Events API Lambda (/events): $EVENTS_API_LAMBDA_NAME"
echo "Split Lambdas:"
echo "  Detector:    $DETECTOR_LAMBDA_NAME (handler: $DETECTOR_HANDLER, mem: ${DETECTOR_MEMORY}MB)"
echo "  Render:      $RENDER_LAMBDA_NAME   (handler: $RENDER_HANDLER, mem: ${RENDER_MEMORY}MB)"
echo "  Events+SNS:  $EVENTS_SNS_LAMBDA_NAME (handler: $EVENTS_SNS_HANDLER, mem: ${EVENTS_SNS_MEMORY}MB)"
...
echo "  GET   $API_ENDPOINT/events        (JWT Auth -> Events API Lambda: $EVENTS_API_LAMBDA_NAME)"
echo "  PATCH $API_ENDPOINT/events        (JWT Auth -> Events API Lambda: $EVENTS_API_LAMBDA_NAME)"
echo "=============================="
echo
echo "IMPORTANT ✅ SNS:"
echo "  1) Check $ADMIN_EMAIL inbox and click CONFIRM subscription (required)"
echo "  2) Events+SNS Lambda env var set: SNS_TOPIC_ARN=$SNS_TOPIC_ARN"
echo "  3) Detector Lambda env vars set:"
echo "       RENDER_LAMBDA_NAME=$RENDER_LAMBDA_NAME"
echo "       EVENTS_LAMBDA_NAME=$EVENTS_SNS_LAMBDA_NAME"
echo "=============================="
echo
